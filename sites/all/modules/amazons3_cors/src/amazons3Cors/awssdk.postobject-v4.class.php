 ...
 */
class PostObjectV4 extends \Aws\S3\Model\PostObject
{
    ...
    public function __construct(S3Client $client, $bucket, array $options = array())
    {
        ...
        parent::__construct($this->client, $this->bucket, $options);
    }
    ...
    public function prepareData()
    {
        ...
        $this->formInputs = array(
          #   'AWSAccessKeyId' => $this->client->getCredentials()
          #->getAccessKeyId()
        );
        ...
        *********/
        ...
        //need $scope? or it happens in signature creation interanlly
        $scope = "$sdt/$region/s3/aws4_request";
        $creds_scope = "{$this->client->getCredentials()->getAccessKeyId()}/$scope";

        $policy['conditions'][] = ['X-Amz-Credential' => $creds_scope];
        ...
        *********/
        ...
        $this->applyPolicy($sdt, $ldt, $region, $creds_scope);
     ...
     */
    protected function applyPolicy($shortDate, $longDate, $region, $credentialScope)
    {
